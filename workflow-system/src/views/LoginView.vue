      ...
      <div class="login-content">
        <div class="login-frame">
          <div>
            ...
            <a class="w-full text-center pt-[15px] text-[13px]" @click="forgotPassword()"
  ...
  methods: {
    async userLogin() {
      try {
        ...
        const archived = user.get("archived");
        const orgName = user.get("orgName");

        if (role === "user") {
          this.role = role;
          this.redirect = this.$route.query.redirect || "/userdashboard";
        } else if (role === "orgUser" && archived === false && orgName) {
          this.role = role;
          this.redirect = this.$route.query.redirect || "/orgDashboard";
        } else if (role === "orgUser" && archived === true && orgName) {
          alert(
            "Your account is archived. Please contact your administrator."
          );
        } else if (role === "admin") {
          this.role = role;
          ...
          this.role = role;
          this.redirect = this.$route.query.redirect || "/moderatorDashboard";
        }
